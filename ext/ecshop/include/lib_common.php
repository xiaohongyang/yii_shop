 ...
 * Time: 10:25
 */
use app\modules\adminshop\models\Region;
...
function assign_query_info()
{
   /* if ($GLOBALS['db']->queryTime == '')
    ...
    if ($GLOBALS['_LANG']['memory_info'] && function_exists('memory_get_usage'))
    {
        ...
        $memory_info = sprintf($GLOBALS['_LANG']['memory_info'], memory_get_usage() / 1048576);

        Yii::$app->params['memory_info'] = ltrim($memory_info, "，");
    }

    /* 是否启用了 gzip */
    $gzip_enabled = gzip_enabled() ? $GLOBALS['_LANG']['gzip_enabled'] : $GLOBALS['_LANG']['gzip_disabled'];
    Yii::$app->params['gzip_enabled'] = $gzip_enabled;
}
...
function get_regions($type = 0, $parent = 0)
{
    ...
    $region = new Region();
    $rs = $region->get_regions($type, $parent);
    return $rs;
